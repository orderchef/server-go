...
func getBillsWithExtras(c *gin.Context) {
	db := database.Mysql()
	...
	for i, bill := range bills {
		if _, err := db.Select(&bills[i].Extras, "select order__bill_extra.quantity, cbi.* from order__bill_extra join config__bill_item as cbi on cbi.id=order__bill_extra.bill_item_id where order__bill_extra.bill_id=?", bill.ID); err != nil {
			panic(err)
		}
	}
